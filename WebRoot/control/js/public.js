...
function initTable(){
	if(document.getElementById("contentTable")){
		senfe("contentTable","#f8fbfc","#e5f1f4","#ecfbd4","#bce774");
	}
}
/**
 * 改变表格窗口颜色
...
function senfe(o,a,b,c,d){
	...
	for(var i=1;i<t.length;i++){
		...
		var select = false;
		if(inputs[0]!=null && inputs[0].type == "checkbox"){
			select = inputs[0].checked;
		}

		for(var j=0;j<tds.length;j++){
			if(select){
				tds[j].style.backgroundColor=d;
				t[i].x="1";
			}else{
				tds[j].style.backgroundColor=(t[i].sectionRowIndex%2==0)?a:b;
				t[i].x="0";
			}
		}
		...
		t[i].onclick=function(){
			...
			if(this.x!="1"){
				...
				var tds = this.getElementsByTagName("td");
				for(var i=0;i<tds.length;i++){
					tds[i].style.backgroundColor=(this.sectionRowIndex%2==0)?a:b;
				}
				var inputs = tds[0].getElementsByTagName("input");
				if(inputs[0]!=null &&inputs[0].type == "checkbox"){
...
function selectAll(field,name){
	...
	if(name){
		idCheckboxs = document.getElementsByName(name);
	}else{
		idCheckboxs = document.getElementsByName("id");
	}
	for(var i=0; i<idCheckboxs.length; i++){
